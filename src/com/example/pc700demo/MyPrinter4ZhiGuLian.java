 ...
 * @author YonKun
 */
public class MyPrinter4ZhiGuLian {
	
	private SerialPort mSerialPort = null;
	private String choosed_serial ="/dev/ttySAC3";
	private int choosed_buad = 38400;

	private static MyPrinter4ZhiGuLian myPrinter = null;
	 ...
	 * @return
	 */
	public synchronized static MyPrinter4ZhiGuLian getMyPrinter() {
		if(myPrinter == null)
			myPrinter = new MyPrinter4ZhiGuLian();
		return myPrinter;
	}
	/**
	 * 打印收据票头
	...
	private synchronized void StartTitlePrinter(int which) {
	    ...
	    try {
			...
			/* 打印Title */
		    /* 字体放大 */
		    printWrite(PrintCommand.getCmdTS((byte)0x01));
		    /* 字体加粗 */
		    ...
		    /* 恢复默认字体 */
		    printWrite(PrintCommand.getCmdTS((byte)0x00));
		    /* 字体加粗 */
		    printWrite(PrintCommand.getCmdN((byte)0x01));
		    /* 向前走纸2行 */
...
///////////////////////////////////===========================///////////////////////////////////
	...
	private void printWrite(byte [] tmpByte) {
		try {
			mSerialPort.getOutputStream().write(tmpByte);
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IOException e) {
